...
std::vector<long> PmergeVector::jacobsthal(size_t size)
{
	std::vector<long> j;
	size_t range = 0;
...
void PmergeVector::displayInfo()
{
	...
	<< " element with std::vector : " << (float)end / CLOCKS_PER_SEC << "sec" << std::endl << std::endl;
}
...
PmergeDeque::PmergeDeque() : _order(0)
{
	
}

PmergeDeque::PmergeDeque(char *input) : _order(1)
{
	if (!input || !input[0])
		throw std::runtime_error("Error : empty string.");
	_start = clock();
...
PmergeDeque &PmergeDeque::operator=(const PmergeDeque& src)
{
	if (this != &src)
	{
		_data = src._data;
...
void PmergeDeque::printDeq()
{
	dit start = _data.begin();
	dit end = _data.end();
	while (start != end)
	{
		std::cout << *start << " ";
...
void PmergeDeque::printDeq(dit begin, dit end)
{
	while (begin != end)
	{
		std::cout << *begin << " ; ";
...
std::deque<long> PmergeDeque::jacobsthal(size_t size)
{
	std::deque<long> j;
	size_t range = 0;
...
void PmergeDeque::exchangePair(int i, int j)
{
	int len = _order - 1;
	int swap = 0;
...
void PmergeDeque::binarySearch(di &main, dit sbegin, dit send, dit start, dit end)
{
	int range = std::distance(start, end + 1) / _order;
	dit middle = end - 1;
...
void PmergeDeque::jacobsthalBinary(di &main, di &pend, std::deque<long>::iterator itj)
{
	while (!pend.empty())
	{
		size_t nbrInsert = *itj - (*(itj - 1));
		...
		{
			int jRange = (*itj + (*(itj - 1))) - 1;
			dit sbegin = pend.begin() + (_order * nbrInsert) - _order;
			dit send = pend.begin() + (_order * nbrInsert) - 1;
			dit start = main.begin();
			int range = (main.size() / _order) - jRange;
			dit end = main.end() - (range * _order) - 1;
			binarySearch(main, sbegin, send, start, end);
			nbrInsert--;
			pend.erase(sbegin, send + 1);
		}
...
void PmergeDeque::binarySort(di &main, di& pend, di &trash)
{
	std::deque<long> j = jacobsthal(pend.size() / _order);
	
	std::deque<long>::iterator ite = std::find(j.begin(), j.end(), 3);
	if (ite == j.end())
		standardBinary(main, pend);
	else
		jacobsthalBinary(main, pend, ite);
...
void PmergeDeque::displayInfo()
{
	...
	std::cout << "Is sorted : " << isSorted(_data.begin(), _data.end()) << std::endl;
	std::cout << "Time to process a range of " << _data.size() 
	<< " element with std::deque : " << (float)end / CLOCKS_PER_SEC << "sec" << std::endl << std::endl;
}
